...
- **Avut (hints panel)**: Collapsible section visible to all players. Contains four individually activatable hints, identified by string IDs — once unlocked they persist in `hintsUnlocked` (Set) in `sanakenno_state` localStorage across sessions. Each hint has a unique icon used in the title and share text:
  1. **`summary`** 📊 — "Yleiskuva": two-line summary. Line 1: remaining/total word count + percentage + remaining/total pangram count. Line 2: number of distinct word lengths + length of the longest word in the puzzle.
