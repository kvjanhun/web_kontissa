...
- **Static caching**: the Flask catch-all (`app/routes.py`) serves `_nuxt/` and `fonts/` with `Cache-Control: public, max-age=31536000, immutable` and lets everything else revalidate. `immutable` is load-bearing, not decoration — plain `max-age` still lets a browser send a conditional request on an explicit reload, which repeats the webfont round trip and repaints the page from the fallback font. `_nuxt/` filenames carry a content hash so a stale cache is impossible; **font filenames do not, so replacing a font needs a new filename** or clients keep the bytes they already hold for a year. Prerendered HTML must keep revalidating: every build rewrites it under the same name, so caching it would pin visitors to the previous release.
