...
- **Avut (hints panel)**: Collapsible section visible to all players. Contains four individually activatable hints, identified by string IDs — once unlocked they persist in `hintsUnlocked` (Set) in `sanakenno_state` localStorage across sessions. Each hint has a unique icon (SVG in UI, emoji in share text):
