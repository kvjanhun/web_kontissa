...
- **Adding puzzles**: Via admin UI: use the Kennotyökalu editor (enter letters, preview, select center, save to any non-live slot). Via code: add an entry to `PUZZLES` in `app/api/kenno.py` with `{"letters": [7 sorted letters]}` and append the desired default center to `_DEFAULT_CENTERS`. Word filtering and scoring are automatic on first access. Cycle length equals `_total_puzzles()` (currently at minimum 41).
