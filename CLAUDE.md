...
- **Schema changes are hand-managed**: there is no Alembic/Flask-Migrate. `db.create_all()` runs on startup to create missing tables for fresh DBs, but it never alters existing ones. Add or change columns on the live DB by hand on the NUC (`sqlite3 /app/data/site.db "ALTER TABLE ..."`) and reflect the change in the corresponding model. Do not add auto-migration code on import.
