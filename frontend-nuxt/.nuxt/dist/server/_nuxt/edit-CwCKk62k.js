...
import { ssrRenderAttrs, ssrRenderStyle, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderList, ssrRenderComponent } from "vue/server-renderer";
      ...
      if (unref(loading)) {
        ...
        ssrRenderList(unref(ingredients), (ing, i) => {
          _push(`<div class="flex gap-2 mb-2"><input${ssrRenderAttr("value", ing.amount)} type="text"${ssrRenderAttr("placeholder", unref(t)("recipeForm.amount"))} class="w-20 px-3 py-2 rounded-lg text-sm outline-none" style="${ssrRenderStyle({
            ...
            border: "1px solid var(--color-border)",
            color: "var(--color-text-primary)"
          })}"><button type="button" class="px-2 text-red-500 hover:bg-red-500/10 rounded"${ssrIncludeBooleanAttr(unref(ingredients).length === 1) ? " disabled" : ""}> × </button></div>`);
        });
        ...
        ssrRenderList(unref(steps), (step, i) => {
          _push(`<div class="flex gap-2 mb-2"><span class="py-2 text-sm w-6 text-right" style="${ssrRenderStyle({ color: "var(--color-text-secondary)" })}">${ssrInterpolate(i + 1)}.</span><textarea${ssrRenderAttr("placeholder", unref(t)("recipeForm.stepPlaceholder"))} rows="2" class="flex-1 px-3 py-2 rounded-lg text-sm outline-none resize-y" style="${ssrRenderStyle({
            ...
            border: "1px solid var(--color-border)",
            color: "var(--color-text-primary)"
          })}">${ssrInterpolate(step.content)}</textarea><button type="button" class="px-2 text-red-500 hover:bg-red-500/10 rounded self-start mt-2"${ssrIncludeBooleanAttr(unref(steps).length === 1) ? " disabled" : ""}> × </button></div>`);
        });
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("pages/recipes/[slug]/edit.vue");
  return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
export {
  _sfc_main as default
};
//# sourceMappingURL=edit-CwCKk62k.js.map
