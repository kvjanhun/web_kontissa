import { resolveComponent, ref, mergeProps, unref, createVNode, resolveDynamicComponent, computed, useSSRContext } from 'file:///Users/erezac/Projects/web_kontissa/frontend-nuxt/node_modules/vue/index.mjs';
import { ssrRenderAttrs, ssrRenderStyle, ssrInterpolate, ssrRenderList, ssrRenderAttr, ssrRenderClass, ssrRenderVNode, ssrIncludeBooleanAttr, ssrLooseContain, ssrLooseEqual, ssrRenderComponent } from 'file:///Users/erezac/Projects/web_kontissa/frontend-nuxt/node_modules/vue/server-renderer/index.mjs';
...
const _sfc_main$7 = {
  ...
  setup(__props) {
    ...
    function xPos(i) {
      var _a;
      const len = ((_a = events.value) == null ? void 0 : _a.series.length) || 1;
...
const _sfc_main$2 = {
  ...
  props: {
    ...
    emptyMessage: { type: String, default: "Ei sanoja t\xE4lle pelille." }
  },
  emits: ["block"],
  setup(__props, { emit: __emit }) {
    const props = __props;
    ...
    return (_ctx, _push, _parent, _attrs) => {
      if (__props.loading) {
        ...
        _push(`<div${ssrRenderAttrs(_attrs)}><p class="text-xs mb-2" style="${ssrRenderStyle({ color: "var(--color-text-tertiary)" })}">${ssrInterpolate(sortedWords.value.length)} sanaa \u2014 klikkaa \xD7 poistaaksesi sanan pysyv\xE4sti </p><div class="flex flex-wrap gap-x-6 gap-y-2"><!--[-->`);
        ssrRenderList(wordColumns.value, (col, ci) => {
          _push(`<ul><!--[-->`);
          ssrRenderList(col, (word) => {
            _push(`<li class="flex items-center gap-1 text-sm py-0.5"><span style="${ssrRenderStyle({ color: isPangram(word) ? "var(--color-accent)" : "var(--color-text-secondary)", fontFamily: "var(--font-mono)", fontWeight: isPangram(word) ? "600" : "normal" })}">${ssrInterpolate(word)}</span><button class="text-xs leading-none opacity-40 hover:opacity-100" style="${ssrRenderStyle({ "color": "#ef4444", "background": "none", "border": "none", "cursor": "pointer", "padding": "0 2px" })}" aria-label="Block word">\xD7</button></li>`);
          });
          _push(`<!--]--></ul>`);
        });
        _push(`<!--]--></div></div>`);
...
const _sfc_main$1 = {
  ...
  __ssrInlineRender: true,
  setup(__props) {
    const FINNISH_LETTERS = new Set("abcdefghijklmnopqrstuvwxyz\xE4\xF6");
    ...
    const allSlotRows = computed(() => {
      var _a;
      const total = (_a = totalPuzzles.value) != null ? _a : 0;
      const rows = [];
    ...
    async function fetchPuzzle(slot) {
      ...
      try {
        ...
        words.value = (_a = data.words) != null ? _a : [];
        totalPuzzles.value = data.total_puzzles;
        return true;
      } catch {
        wordsError.value = "Pelin lataus ep\xE4onnistui.";
        words.value = [];
        return false;
      } finally {
    ...
    async function fetchVariations(slot) {
      ...
      try {
        ...
        variationsError.value = "Variaatioiden lataus ep\xE4onnistui.";
        variations.value = [];
        return false;
      } finally {
    ...
    async function setCenter(letter) {
      ...
      try {
        ...
        variationsError.value = "Keskuskirjaimen vaihto ep\xE4onnistui.";
      } finally {
        centerSaving.value = false;
      }
    }
    async function blockWord(word) {
      if (!confirm(`Poista "${word}" sanalistalta pysyv\xE4sti?`)) return;
      try {
        ...
        wordsError.value = "Sanan poisto ep\xE4onnistui.";
      }
    }
    async function fetchPreview() {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.error || "Esikatselu ep\xE4onnistui");
        }
        const data = await res.json();
        variations.value = data.variations.map((v) => ({ ...v, is_active: false }));
      } catch (e) {
        variationsError.value = e.message || "Esikatselu ep\xE4onnistui.";
        variations.value = [];
      } finally {
        variationsLoading.value = false;
      }
    ...
    async function selectPreviewCenter(letter) {
      ...
      try {
        ...
        if (!res.ok) {
          if (res.status === 429) {
            wordsError.value = "Liian monta pyynt\xF6\xE4 \u2014 odota hetki.";
          }
          throw new Error();
        }
        const data = await res.json();
        ...
        if (!wordsError.value) wordsError.value = "Sanalistan lataus ep\xE4onnistui.";
        words.value = [];
      } finally {
        wordsLoading.value = false;
      }
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (allSlotRows.value.length > 0) {
        ...
        ssrRenderList(allSlotRows.value, (row) => {
          ...
          if (row.isToday) {
            _push(`<span class="px-1 rounded shrink-0" style="${ssrRenderStyle({ "background": "#ef4444", "color": "white", "font-size": "0.625rem", "line-height": "1.4" })}">t\xE4n\xE4\xE4n</span>`);
          } else {
            _push(`<!---->`);
          }
      ...
      _push(`</div><div class="flex-1 min-w-0"><div class="flex flex-wrap items-center gap-x-3 gap-y-2 mb-3"><div class="flex items-center gap-1"><label class="text-xs" style="${ssrRenderStyle({ color: "var(--color-text-secondary)" })}">Peli</label><input type="number"${ssrRenderAttr("value", displayNumber.value)} min="1"${ssrRenderAttr("max", (_a = totalPuzzles.value) != null ? _a : 999)} class="rounded text-center" style="${ssrRenderStyle({ "width": "3.5rem", "background": "var(--color-bg-secondary)", "color": "var(--color-text-primary)", "border": "1px solid var(--color-border)", "padding": "2px 4px", "font-size": "0.875rem" })}"><span class="text-xs" style="${ssrRenderStyle({ color: "var(--color-text-tertiary)" })}">/${ssrInterpolate((_b = totalPuzzles.value) != null ? _b : "\u2026")}</span></div><div class="flex items-center gap-1"><input type="text"${ssrRenderAttr("value", lettersInput.value)} placeholder="kirjaimet" maxlength="14" class="rounded" style="${ssrRenderStyle({ "width": "7rem", "background": "var(--color-bg-secondary)", "color": "var(--color-text-primary)", "border": "1px solid var(--color-border)", "padding": "2px 6px", "font-size": "0.875rem", "font-family": "var(--font-mono)" })}"><button class="rounded text-xs px-1.5 py-0.5" style="${ssrRenderStyle({ background: "var(--color-bg-secondary)", color: "var(--color-text-tertiary)", border: "1px solid var(--color-border)" })}">Tyhjenn\xE4</button>`);
      ...
      _push(`</div><div class="flex items-center gap-1"><span class="text-xs" style="${ssrRenderStyle({ color: "var(--color-text-secondary)" })}">\u2194</span><input type="number"${ssrRenderAttr("value", swapSlotInput.value)} min="1"${ssrRenderAttr("max", (_c = totalPuzzles.value) != null ? _c : 999)} class="rounded text-center" style="${ssrRenderStyle({ "width": "3.5rem", "background": "var(--color-bg-secondary)", "color": "var(--color-text-primary)", "border": "1px solid var(--color-border)", "padding": "2px 4px", "font-size": "0.875rem" })}"><button${ssrIncludeBooleanAttr(!canSwap.value || swapLoading.value) ? " disabled" : ""} class="rounded text-xs px-1.5 py-0.5" style="${ssrRenderStyle({
      ...
      _push(`</div>`);
      if (isToday.value) {
        _push(`<p class="text-xs mb-2" style="${ssrRenderStyle({ "color": "#ef4444" })}">T\xE4m\xE4n p\xE4iv\xE4n peli\xE4 ei voi muokata.</p>`);
      } else {
        _push(`<!---->`);
      }
      if (swapError.value) {
      ...
      ssrRenderList(7, (i) => {
        ...
        })}">${ssrInterpolate((_b2 = (_a2 = parsedLetters.value[i - 1]) == null ? void 0 : _a2.toUpperCase()) != null ? _b2 : "")}</div>`);
      });
      ...
      if (isDirty.value) {
        ...
        if (variationsLoading.value) {
          ...
          _push(ssrRenderComponent(_sfc_main$3, {
            variations: variations.value,
            "active-center": (_d = savedCenter.value) != null ? _d : "",
...
//# sourceMappingURL=admin-iLSzzwzd.mjs.map
