...
Before reporting success, verify the single-owner docs still tell the truth
about this change (see the Documentation Upkeep table in `CLAUDE.md`):

- Stack, commands, gates, or security rules changed → `CLAUDE.md`.
- Request flow or auth changed → `docs/architecture.md`.
- Deploy or backup/restore changed → `server/README.md`.
- Anything host-level (nginx, firewall, cron, webhook) → the `nuc` repo, not
  this one.
