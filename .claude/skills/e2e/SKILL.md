...
Standalone Playwright runner. Handles the recurring footgun documented in `CLAUDE.md`: `playwright.config.js` has `reuseExistingServer: !process.env.CI`, so any dev Flask listening on :5001 with `site.db` will be reused instead of the test-configured Flask, and DB-backed specs (auth, admin, recipes) will silently fail.
