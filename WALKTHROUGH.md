...
- After registering routes, imports `kenno` (and other API modules) to register URL routes
...
- **Puzzles**: All puzzles are DB-backed in the `KennoPuzzle` table. Center letters are stored in `KennoConfig` (key `center_{idx}`). For a fresh DB, run `scripts/seed_puzzles.py`.
