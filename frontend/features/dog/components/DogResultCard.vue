...
<template>
  <div class="dog-result-card" :class="[gradeBorderClass(dog.grade), critiqueExpanded && 'dog-result-card-expanded']">
    <div class="dog-result-row">
      ...
      <span v-if="showAwardRank && dog.awardRank" class="dog-placement-badge" title="Sijoitus">{{ dog.awardRank }}.</span>
      <span v-else-if="!showAwardRank && dog.placement" class="dog-placement-badge" title="Sijoitus">{{ dog.placement }}.</span>
      ...
      <span v-if="showInlineMeta && dog.gender" :class="['dog-gender-badge-inline', dog.gender]">
        {{ dog.gender === 'uros' ? '♂' : dog.gender === 'narttu' ? '♀' : '' }}
      </span>

      <!-- Dog name col -->
      <div class="dog-name-col">
        <a
          v-if="safeRegUrl"
          :href="safeRegUrl"
          target="_blank"
