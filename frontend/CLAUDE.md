...
- **Sections can be hidden from the admin.** `home.hiddenSections` arrives in the same overlay as the copy; `composables/useHomeSections.js` turns it into `isVisible(key)` and `HIDEABLE_SECTIONS` mirrors `HOME_SECTIONS` in `app/home_content.py`. Every anchor pointing at a band must be gated on the same check, or it scrolls nowhere: the header nav and drawer links, the hero CTAs, and the project reach chips in `HomeWork` (which degrade to plain `<span>`s).
...
- **Homepage**: its own cooler near-black/cream palette + IBM Plex Sans/Mono typography, all **scoped to the `.home-dc` wrapper** in `assets/style.css` (`--bg`/`--panel`/`--line`/`--tx`/`--accent` etc.). IBM Plex woff2 (latin subset) is self-hosted in `public/fonts/`. **Sans is one variable face spanning `font-weight: 100 700`** — it is a variable font, so a face per weight would point several URLs at identical bytes and fetch the same file that many times; Mono is three genuinely distinct cuts and stays three faces. `pages/index.vue` preloads the Sans file via `useHead` (scoped there because no other page uses it) so `font-display: swap` does not paint the `system-ui` fallback first, whose metrics differ enough to reflow the page as the real font lands. Swapping any font file needs a new filename — see the static-caching note in the root `CLAUDE.md`. The rest of the site is unaffected.
- **Texture + readability plate**: `.home-dc` paints a faint graph-paper grid (`--grid` linear-gradient lines) behind everything not on an opaque panel. Text that sits directly on that grid (hero eyebrow/title/body, section labels, project heads/details, stack intro) carries the `.home-plate` utility — a `::before` filled with `var(--bg)` (same colour as the page, so it only hides the local grid lines, never reads as a box) masked by the **intersection of a horizontal and a vertical linear gradient** (`mask-composite: intersect`, with `-webkit-mask-composite: source-in` fallback), so the fill feathers to transparent on every edge and the texture re-emerges around each block. Panels (stack table, terminal frame, screenshots) and the footer (`--bg-2`) are already opaque, so they don't need it.
