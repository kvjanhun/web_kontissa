...
- **Layouts**: `default.vue` (header/footer, calls `checkAuth` on mount) and `standalone.vue` (bare, used by the home page via `definePageMeta({ layout: 'standalone' })`).
...
- Accessibility: skip-to-content, `:focus-visible` rings, `aria-expanded` menu, `aria-live` announcer, `prefers-reduced-motion` respected
