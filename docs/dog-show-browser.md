...
- `DOG_RESULT_FINALS_PROBE_TTL`: seconds between reads of the `R=RYP` / `R=BIS` pages while a read comes back empty; defaults to `600`. Once a read shows a placement, or every ring is captured, they are read every pass. The pacing follows the last read rather than the accumulated award record, so a multi-day show whose pages were cleared at the day boundary is not re-read every pass through the following morning while they sit empty.
