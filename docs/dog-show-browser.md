...
- **Finals re-sweep (the one exception to immutability).** Show finals (`RYP`/`BIS-1`/`BIS JUN`/`BIS VET`) are appended onto the *winners'* already-captured breed rows after every ring is judged. Once all judged breeds are captured but a main BIS is still expected and not yet recorded, the refresh re-checks a bounded, rotating chunk of captured breeds (`DOG_RESULT_FINALS_SWEEP_BREED_LIMIT`, default 30, cycling via `finals_sweep_cursor`) so the finals land within a few passes instead of a whole-show burst. Re-fetched rows replace the breed's old rows (no duplication). `BIS-1..4` each sit on a *different* winning breed's row and `RYP` is decided before `BIS`, so by the time `BIS-1` appears every finals placement is already published. Stopping the instant `BIS-1` was recorded therefore stranded the later placements (`BIS-4`, late `RYP`) on breed rows the rotation had not reached yet — e.g. a big all-breed show capturing only 3 of 4 `BIS`. So once the first main BIS lands, the sweep budgets **exactly one further full rotation** over the captured breeds (`finals_post_bis_sweep_remaining`, persisted in the cache meta blob so it survives header-only saves) before settling; it also stops if the show drops out of its live date range. Caveat: each day of a multi-day all-breed show crowns its own finals, and a *live* later-day `BIS` published after this budget is spent can still be missed until a forced/backfill re-crawl — settled past shows are re-fetched in full and end up complete.
