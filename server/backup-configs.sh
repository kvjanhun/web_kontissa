...
# fail2ban (exclude the AbuseIPDB API key — it lives in site-alerts.env, never B2)
cp -r /etc/fail2ban "$BACKUP_DIR/" 2>/dev/null || true
rm -f "$BACKUP_DIR/fail2ban/action.d/abuseipdb.local" 2>/dev/null || true

# Crontab (kvjanhun's and root's — the AbuseIPDB blocklist cron is root's)
crontab -u kvjanhun -l > "$BACKUP_DIR/crontab.txt" 2>/dev/null || true
crontab -u root -l > "$BACKUP_DIR/crontab-root.txt" 2>/dev/null || true
...
# iptables + ipset (AbuseIPDB blocklist set)
