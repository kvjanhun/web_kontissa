...
# Day one of a two-day show, running FCI 5 and 6 as one combined ring. Group 6
# wins no RYP-1 of its own and never will, so this page heading is the only
# statement anywhere that the group has been crowned — no breed row can carry it.
SAMPLE_DAY_ONE_COMBINED_RYP_HTML = """
<div id="divContent">
<table class="tulostaulukko">
<tr class="otsikko"><td colspan="3">
  <div class="floatleft">FCI 5/6 - Pystykorvat - Ajavat ja jäljestävät koirat</div>
</td></tr>
<tr><td>1.</td><td>basenji</td>
  <td><a href="https://jalostus.kennelliitto.fi/frmKoira.aspx?RekNo=FI11111%2F20">Basenji Yksi</a> Om. Omistaja A</td></tr>
</table>
...
# Day two of that show. A new group ring is crowned, the main Best in show
# finally lands — and the junior final is re-run, naming a different dog under a
# section heading the page already used yesterday.
SAMPLE_DAY_TWO_RYP_HTML = """
<div id="divContent">
<table class="tulostaulukko">
<tr class="otsikko"><td colspan="3"><div class="floatleft">FCI 3 - Terrierit</div></td></tr>
<tr><td>1.</td><td>skotlanninterrieri</td>
  <td><a href="https://jalostus.kennelliitto.fi/frmKoira.aspx?RekNo=FI33333%2F22">Terrieri Yksi</a> Om. Omistaja C</td></tr>
</table>
</div>
"""

SAMPLE_DAY_TWO_BIS_HTML = """
<div id="divContent">
<table class="tulostaulukko">
...
def _two_day_show_breeds():
    """FCI 5 and 6 share a combined ring on day one; group 3 is judged on day two."""
    return [
        {"name": "basenji", "count": 2, "group": "5", "breed_id": "3", "has_results": True},
...
def _two_day_show_results():
    """Every finals token the weekend's pages promise, on the winners' rows.

    Group 6 holds only its breed winner: the combined ring crowned a group 5 dog,
    so no group 6 row can ever carry an RYP-1."""
    return [
        {"breedGroup": "5", "breedId": "3", "awards": "SA, ROP, RYP-1, BIS-1",
         "reg_url": "https://jalostus.kennelliitto.fi/frmKoira.aspx?RekNo=FI11111%2F20"},
        {"breedGroup": "6", "breedId": "64", "awards": "SA, ROP"},
        {"breedGroup": "3", "breedId": "191", "awards": "SA, ROP, RYP-1, BIS JUN-1",
         "reg_url": "https://jalostus.kennelliitto.fi/frmKoira.aspx?RekNo=FI33333%2F22"},
    ]


def _probe_weekend(monkeypatch, sequence):
    """Run `_probe_finals_pages` once per (RYP html, BIS html) pair, on one doc."""
    from bs4 import BeautifulSoup

    doc = {}
    for ryp_html, bis_html in sequence:
        pages = {"RYP": ryp_html, "BIS": bis_html}
        monkeypatch.setattr(
            dog_result_cache, "_fetch_page",
            lambda url, pages=pages: BeautifulSoup(
                pages["RYP"] if "RYP" in url else pages["BIS"], "html.parser"),
        )
        dog_result_cache._probe_finals_pages(13780, doc)
    return doc


def test_the_finals_probe_keeps_day_ones_awards_when_the_page_is_cleared(monkeypatch):
    """The finals pages are a current-day view, and the source clears them.

    Show 13774 served day one's three side finals on `R=BIS` until 15:44 on
    2026-09-20 and day two's single breeder-group ring two minutes later — a
    retraction in the middle of fetch hours, not an overnight rollover the night
    stop happens to cover. Storing only the latest read loses day one."""
    doc = _probe_weekend(monkeypatch, [
        (SAMPLE_DAY_ONE_COMBINED_RYP_HTML, SAMPLE_SIDE_BIS_ONLY_HTML),
        (SAMPLE_EMPTY_FINALS_PAGE_HTML, SAMPLE_EMPTY_FINALS_PAGE_HTML),   # cleared
        (SAMPLE_DAY_TWO_RYP_HTML, SAMPLE_DAY_TWO_BIS_HTML),
    ])

    probe = dog_finals.probe_state(doc, _two_day_show_breeds())
    # Day one's combined ring survived the clearing; day two's ring was added.
    assert probe["ryp_groups_awarded"] == ["3", "5", "6"]
    assert probe["main_bis_awarded"] is True
    assert probe["published"] is True


def test_a_cleared_page_leaves_a_combined_ring_group_uncrowned_for_good():
    """Why the accumulation is load-bearing and not merely tidy.

    A combined `FCI 5/6` ring is the one crowning no breed row can testify to —
    group 6 wins no RYP-1 of its own — so once the page that said so is cleared,
    the group is uncrowned for the rest of the show and the ladder's first rung
    can never fire again."""
    breeds = _two_day_show_breeds()
    doc = {
        "results": _two_day_show_results(),
        # Day two's pages only: what wholesale replacement leaves behind.
        "finals_probe": _finals_probe(SAMPLE_DAY_TWO_RYP_HTML, SAMPLE_DAY_TWO_BIS_HTML),
    }
    analysis = dog_finals.analyze(doc, breeds)
    assert analysis["probe"]["ryp_groups_awarded"] == ["3"]
    assert analysis["target_met"] is False, "group 6 has entries and no ring on record"

    # The same weekend accumulated: the combined ring is back on the page and
    # accounts for the group no row can.
    doc["finals_probe"]["pages"]["RYP"]["sections"] = dog_finals.merge_probe_sections(
        _finals_probe(SAMPLE_DAY_ONE_COMBINED_RYP_HTML)["pages"]["RYP"]["sections"],
        doc["finals_probe"]["pages"]["RYP"]["sections"],
    )
    analysis = dog_finals.analyze(doc, breeds)
    assert analysis["probe"]["ryp_groups_awarded"] == ["3", "5", "6"]
    assert analysis["target_met"] is True


def test_an_empty_ryp_page_does_not_excuse_a_group_that_was_never_crowned():
    """An empty RYP page is not a statement that no group is outstanding.

    The check is suspended only when nothing anywhere has crowned a group, which
    is the specialty cluster that runs no group stage at all. An all-breed show
    whose page is empty still owes every group its rows have not accounted for —
    otherwise publishing the main BIS alone would settle it."""
    breeds = _two_day_show_breeds()
    doc = {
        "results": _two_day_show_results(),
        "finals_probe": _finals_probe(SAMPLE_EMPTY_FINALS_PAGE_HTML, SAMPLE_DAY_TWO_BIS_HTML),
    }
    analysis = dog_finals.analyze(doc, breeds)
    assert analysis["probe"]["main_bis_awarded"] is True
    assert analysis["target_met"] is False, "groups 5 and 3 crowned on rows, 6 on nothing"

    # A show with no group stage anywhere is the shape the suspension is for, and
    # it still settles on its BIS alone.
    cluster = {
        "results": [{"breedGroup": "5", "breedId": "3", "awards": "SA, ROP, BIS-1",
    ...
    assert dog_finals.analyze(cluster, breeds)["target_met"] is True


def test_a_refilled_finals_section_replaces_the_day_it_supersedes(monkeypatch):
    """Accumulating is per heading, not per placement: day two's junior final
    stands in for day one's rather than piling up beside it, so the obligation
    to find a `BIS JUN-1` follows the dog that currently holds it."""
    doc = _probe_weekend(monkeypatch, [
        (SAMPLE_TWO_GROUP_RYP_HTML, SAMPLE_SIDE_BIS_ONLY_HTML),
        (SAMPLE_DAY_TWO_RYP_HTML, SAMPLE_DAY_TWO_BIS_HTML),
    ])
    headings = [s["heading"] for s in doc["finals_probe"]["pages"]["BIS"]["sections"]]
    assert headings == ["Paras juniori", "Best in show"]

    # Day one's junior winner (the basenji) is no longer owed the token; day
    # two's (the terrier) is, and its row has it.
    doc["results"] = _two_day_show_results()
    assert dog_finals.probe_state(doc, _two_day_show_breeds())["missing_keys"] == []

    doc["results"] = [r for r in _two_day_show_results() if r["breedGroup"] != "3"]
...
def test_the_probe_cadence_follows_the_page_not_the_accumulated_record(monkeypatch):
    ...
    cleared for the night every two minutes until the next day's rings start is
    exactly the load this probe exists to avoid."""
    doc = _probe_weekend(monkeypatch, [
        (SAMPLE_TWO_GROUP_RYP_HTML, SAMPLE_SIDE_BIS_ONLY_HTML),
    ])
    assert dog_result_cache._finals_probe_placements(doc) == 3
    assert dog_result_cache._finals_probe_due(doc, True, False) is True

    doc = _probe_weekend(monkeypatch, [
        (SAMPLE_TWO_GROUP_RYP_HTML, SAMPLE_SIDE_BIS_ONLY_HTML),
        (SAMPLE_EMPTY_FINALS_PAGE_HTML, SAMPLE_EMPTY_FINALS_PAGE_HTML),
    ])
    assert doc["finals_probe"]["pages"]["RYP"]["sections"], "the record still holds day one"
    assert dog_result_cache._finals_probe_placements(doc) == 0
    assert dog_result_cache._finals_probe_due(doc, True, False) is False
