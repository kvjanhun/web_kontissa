...
class TestTrackPageview:
    ...
    def test_increments_new_path(self, client):
        res = client.post("/api/pageview", json={"path": "/recipes"})
    ...
    def test_dedup_same_session(self, client):
        ...
        client.post("/api/pageview", json={"path": "/recipes"})
        res = client.post("/api/pageview", json={"path": "/recipes"})
    ...
    def test_different_paths_counted_separately(self, client):
        ...
        assert res.get_json()["count"] == 1
        # Second hit on /recipes still deduped
        res2 = client.post("/api/pageview", json={"path": "/recipes"})
    ...
    def test_different_sessions_counted(self, app):
        ...
        c1 = app.test_client()
        c2 = app.test_client()
        c1.post("/api/pageview", json={"path": "/recipes"})
        res = c2.post("/api/pageview", json={"path": "/recipes"})
...
class TestListPageviews:
    ...
    def test_admin_gets_list(self, app, logged_in_admin):
        ...
        c3 = app.test_client()
        c1.post("/api/pageview", json={"path": "/recipes"})
        c2.post("/api/pageview", json={"path": "/recipes"})
        ...
        data = res.get_json()
        assert len(data) == 2
        # Sorted by count desc — /recipes (2) first
        assert data[0]["path"] == "/recipes"
...
class TestPageviewEvents:
    ...
    def test_records_events(self, app, logged_in_admin):
        ...
        data = res.get_json()
        assert "/recipes" in data["paths"]
        # Find today's entry
        today_entry = data["series"][-1]
        assert today_entry["counts"].get("/recipes") == 2
    ...
    def test_multiple_paths(self, app, logged_in_admin):
        ...
        res = logged_in_admin.get("/api/pageviews/events?days=1")
        data = res.get_json()
        assert sorted(data["paths"]) == ["/about", "/recipes"]
